...
3. **승인 요청**: `curl -s -X POST http://localhost:4500/api/tasks -H 'Content-Type: application/json' -d '{"title":"...","detail":"무엇을/왜/예상 변경 파일"}'` → 응답의 `id` 기억.
4. **결정 대기**: `GET /api/tasks/<id>`를 30초 간격으로 최대 30분 폴링.
   - `approved` → 5단계 진행.
...
6. **결과 보고**: `POST /api/info`로 한 일 요약(브랜치명·커밋·다음 단계). `REQUESTS.md` 진행 로그도 갱신.
